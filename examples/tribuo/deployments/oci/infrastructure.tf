...
resource "oci_core_vcn" "ocivcn" {
  ...
  display_name   = "ociVcn-${var.suffix_name}"
  dns_label      = "ocivcn${var.suffix_name}"
}

resource "oci_core_security_list" "oci_hosts_security_list" {
  compartment_id = "${var.compartment_ocid}"
  vcn_id         = "${oci_core_vcn.ocivcn.id}"
  display_name   = "ociHostsSecurityLists-${var.suffix_name}"
...
resource "oci_core_internet_gateway" "oci_internet_gateway" {
  compartment_id = "${var.compartment_ocid}"
  display_name   = "ociInternetGateway-${var.suffix_name}"
  vcn_id         = "${oci_core_vcn.ocivcn.id}"
}
...
resource "oci_core_route_table" "oci_route_table" {
  compartment_id = "${var.compartment_ocid}"
  vcn_id         = "${oci_core_vcn.ocivcn.id}"
  display_name   = "ociRouteTable-${var.suffix_name}"
...
resource "oci_core_subnet" "oci_subnet" {
  ...
  display_name        = "ociSubnet-${var.suffix_name}"
  dns_label           = "ocisubnet"
...
resource "oci_core_instance" "oci_oci_instance" {
  availability_domain = "${data.oci_identity_availability_domain.ad.name}"
  compartment_id      = "${var.compartment_ocid}"
  display_name        = "ociHost-${var.suffix_name}"
  ...
  create_vnic_details {
    ...
    hostname_label   = "ociHost-${var.suffix_name}-${count.index}"
  }
