...
isVersionGreaterThanOrEqualTo() {
  result="greaterThan"      ### returning a value means true
  if [[ "$1" = "$2" ]]; then
  	result="equalTo"      ### returning a value means true
  else
  	lesser_version="$(echo "$@" | tr " " "\n" | sort -V | head -n 1)"
  	if [[ "${lesser_version}" = "$1" ]]; then # the first version is lesser than the second
